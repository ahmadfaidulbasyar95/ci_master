...
{	
	...
	public function getForm($index = '', $values = array())
	{
		...
		if ($this->isPlainText) {
			...
			$form .= '<textarea name="'.$name.'" class="form-control '.$this->attr_class.' '.str_replace('{name}', $this->name.'_'.$index, $this->toolHtmlEditor).'" title="'.$this->caption.'" placeholder="'.$this->caption.'" '.$this->attr.' '.$this->isRequire.'>'.$this->getValue($index).'</textarea>';
		}
